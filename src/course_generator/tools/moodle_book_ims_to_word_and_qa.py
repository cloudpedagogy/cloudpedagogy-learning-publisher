...
BUILD_ROOT = PROJECT_ROOT / "build" / "moodle_books"
OUTPUT_ROOT = PROJECT_ROOT / "output" / "moodle_books"
...
def ensure_dirs() -> None:
    """Create the shared Moodle Book build and output roots."""
    for folder in [BUILD_ROOT, OUTPUT_ROOT]:
...
def extract_zip(zip_path: Path) -> Path:
    ...
    extracted_dir = BUILD_ROOT / book_id / "extracted_ims"
...
def audit(zip_path: Path, include_optional: bool = False) -> None:
    ensure_dirs()
    ...
    report_dir = OUTPUT_ROOT / book_id / "qa"
...
def export_docx(zip_path: Path, single_docx: bool = False) -> None:
    ensure_dirs()
    validate_zip(zip_path)
    ...
    output_dir = OUTPUT_ROOT / book_id / "docx"
    ...
    if not single_docx:
        ...
        combined_dir = BUILD_ROOT / book_id / "combined"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit Moodle Book IMS ZIPs and export their chapters to Word DOCX")
    ...
    if args.command == "audit":
        audit(Path(args.zip_path), include_optional=args.include_optional)

    if args.command == "export-docx":
        export_docx(Path(args.zip_path), single_docx=args.single_docx)
